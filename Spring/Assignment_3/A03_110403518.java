...
public class A03_110403518 {
    ...
    public static int dfs(ArrayList<int[]> domain, boolean[][] visited, int i, int j, int count, int value, char method, int step) { // O(nm)
        ...
        // 遞迴
        result = Math.max(result, dfs(domain, visited, i - 1, j, count, value, method, step + 1)); // Up
        result = Math.max(result, dfs(domain, visited, i + 1, j, count, value, method, step + 1)); // Down
